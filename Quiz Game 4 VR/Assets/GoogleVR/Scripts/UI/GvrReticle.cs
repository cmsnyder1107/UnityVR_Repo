...
using UnityEngine;
using UnityEngine.EventSystems;//need library functions @@@@@@@@@@@@@@@@@@@@@@@@@@@
...
public class GvrReticle : MonoBehaviour, IGvrGazePointer {
  ...
  private float reticleOuterDiameter = 0.0f;
    ...
    private float gazeStartTime;//keep track of how long we've been gazing @@@@@@@@@@@@@@@@@@@@@@@@@@@
    private GameObject gazedAt;//track the object we've been gazing at @@@@@@@@@@@@@@@@@@@@@@@@@@@
    ...
    materialComp = gameObject.GetComponent<Renderer>().material;
        gazeStartTime = -1f;
        gazedAt = null;//used to avoid any weird exceptions/errors @@@@@@@@@@@@@@@@@@@@@@@@@@@
    }
                          ...
                          bool isInteractive)
  {
    SetGazeTarget(intersectionPosition, isInteractive);
        ...
        gazedAt = targetObject;//track what we're looking at @@@@@@@@@@@@@@@@@@@@@@@@@@@
        gazeStartTime = Time.time;//the time we start looking @@@@@@@@@@@@@@@@@@@@@@@@@@@
  }
                         ...
                         bool isInteractive)
  {
    SetGazeTarget(intersectionPosition, isInteractive);
        ...
        //make sure gazedAt != null to avoid weird reference exceptions
        //check gazeStartTime > 0 to avoid events being fired on the same object more than once
            //will have to reset gazeStartTime = -1 every time the check returns true and the event is fired
        if (gazedAt != null && gazeStartTime > 0f)
        {
            if (Time.time - gazeStartTime > 5.0f && ExecuteEvents.CanHandleEvent<TimedInputHandler>(gazedAt))//we've been looking at least 3 seconds
            {
                gazeStartTime = -1f;//prevents events triggering multiple times on the same object
